...
class GFPayPal {
    ...
    public static function tooltips($tooltips){
        $paypal_tooltips = array(
            ...
            "paypal_recurring_amount" => "<h6>" . __("Recurring Amount", "gravityformspaypal") . "</h6>" . __("Select which field determines the recurring payment amount.", "gravityformspaypal"),
    ...
    private static function list_page(){
        ...
        <div class="wrap">
            ...
            <h2><?php _e("PayPal Forms", "gravityformspaypal"); ?>
    ...
    private static function stats_page(){
        ...
        <script type="text/javascript" src="<?php echo plugins_url("gravityforms") ?>/js/currency.js"></script>
    ...
    private static function edit_page(){
        ...
        if(rgpost("gf_paypal_submit")){
            ...
            $config["meta"]["email"] = rgpost("gf_paypal_email");
        ...
        <form method="post" action="">
            ...
            <div id="paypal_field_group" valign="top" <?php echo empty($config["meta"]["type"]) || empty($config["form_id"]) ? "style='display:none;'" : "" ?>>
                ...
                <div id="paypal_field_container_subscription" class="paypal_field_container" valign="top" <?php echo $config["meta"]["type"] != "subscription" ? "style='display:none;'" : ""?>>
                    <div class="margin_vertical_10">
                    ...
                    <div class="margin_vertical_10">
                        ...
                        <select id="gf_paypal_billing_cycle_type" name="gf_paypal_billing_cycle_type" onchange="SetPeriodNumber('#gf_paypal_billing_cycle_number', jQuery(this).val());">
                            <option value="D" <?php echo $config["meta"]["billing_cycle_type"] == "D" ? "selected='selected'" : "" ?>><?php _e("day(s)", "gravityformspaypal") ?></option>
                            <option value="W" <?php echo $config["meta"]["billing_cycle_type"] == "W" ? "selected='selected'" : "" ?>><?php _e("week(s)", "gravityformspaypal") ?></option>
                            <option value="M" <?php echo $config["meta"]["billing_cycle_type"] == "M" || empty($config["meta"]["billing_cycle_type"]) ? "selected='selected'" : "" ?>><?php _e("month(s)", "gravityformspaypal") ?></option>
                            <option value="Y" <?php echo $config["meta"]["billing_cycle_type"] == "Y" ? "selected='selected'" : "" ?>><?php _e("year(s)", "gravityformspaypal") ?></option>
                        </select>
                    </div>
                    ...
                    <div class="margin_vertical_10">
                        ...
                        <input type="checkbox" name="gf_paypal_recurring_retry" id="gf_paypal_recurring_retry" value="1" <?php echo $config["meta"]["recurring_retry"] ? "checked='checked'" : ""?> />
                    ...
                    <div class="margin_vertical_10">
                        ...
                        <input type="checkbox" name="gf_paypal_trial_period" id="gf_paypal_trial_period" value="1" onclick="if(jQuery(this).is(':checked')) jQuery('#paypal_trial_period_container').show('slow'); else jQuery('#paypal_trial_period_container').hide('slow');" <?php echo $config["meta"]["trial_period_enabled"] ? "checked='checked'" : ""?> />
                        <label class="inline" for="gf_paypal_trial_period"><?php _e("Enable", "gravityformspaypal"); ?></label>
                    </div>

                    <div id="paypal_trial_period_container" <?php echo $config["meta"]["trial_period_enabled"] ? "" : "style='display:none;'" ?>>
                        <div class="margin_vertical_10">
                            ...
                            <input type="text" name="gf_paypal_trial_amount" id="gf_paypal_trial_amount" value="<?php echo $config["meta"]["trial_amount"] ?>"/>
                        </div>
                        <div class="margin_vertical_10">
                            ...
                            <select id="gf_paypal_trial_period_number" name="gf_paypal_trial_period_number">
                                ...
                                for($i=1; $i<=100; $i++){
                                ?>
                                    <option value="<?php echo $i ?>" <?php echo $config["meta"]["trial_period_number"] == $i ? "selected='selected'" : "" ?>><?php echo $i ?></option>
                                <?php
                                }
                                ?>
                            </select>&nbsp;
                            <select id="gf_paypal_trial_period_type" name="gf_paypal_trial_period_type" onchange="SetPeriodNumber('#gf_paypal_trial_period_number', jQuery(this).val());">
                                <option value="D" <?php echo $config["meta"]["trial_period_type"] == "D" ? "selected='selected'" : "" ?>><?php _e("day(s)", "gravityformspaypal") ?></option>
                                <option value="W" <?php echo $config["meta"]["trial_period_type"] == "W" ? "selected='selected'" : "" ?>><?php _e("week(s)", "gravityformspaypal") ?></option>
                                <option value="M" <?php echo $config["meta"]["trial_period_type"] == "M" || empty($config["meta"]["trial_period_type"]) ? "selected='selected'" : "" ?>><?php _e("month(s)", "gravityformspaypal") ?></option>
                                <option value="Y" <?php echo $config["meta"]["trial_period_type"] == "Y" ? "selected='selected'" : "" ?>><?php _e("year(s)", "gravityformspaypal") ?></option>
                            </select>
                        </div>
                ...
                <div class="margin_vertical_10">
                    ...
                    <input type="text" name="gf_paypal_page_style" id="gf_paypal_page_style" class="width-1" value="<?php echo $config["meta"]["style"] ?>"/>
                </div>
                <div class="margin_vertical_10">
                    ...
                    <input type="text" name="gf_paypal_continue_text" id="gf_paypal_continue_text" class="width-1" value="<?php echo $config["meta"]["continue_text"] ?>"/>
                </div>
                <div class="margin_vertical_10">
                    ...
                    <input type="text" name="gf_paypal_cancel_url" id="gf_paypal_cancel_url" class="width-1" value="<?php echo $config["meta"]["cancel_url"] ?>"/>
                </div>
        ...
        <script type="text/javascript">
            ...
            function EndSelectForm(form_meta, customer_fields, recurring_amount_options){
                ...
                else if(type == "donation" && GetFieldsByType(["donation"]).length == 0){
    ...
    public static function send_to_paypal($confirmation, $form, $entry, $ajax){
        ...
        $business_email = urlencode($config["meta"]["email"]);
        $custom_field = $entry["id"] . "|" . wp_hash($entry["id"]);
        $url .= "?notify_url={$ipn_url}&currency_code={$currency}&business={$business_email}&custom={$custom_field}{$invoice}{$customer_fields}{$page_style}{$continue_text}{$cancel_url}{$disable_note}{$disable_shipping}{$return_url}";
        $query_string = "";
    ...
    public static function process_ipn($wp){
        ...
        if(strtolower($_POST["business"]) != $config["meta"]["email"])
            return;
    ...
    private static function get_donation_query_string($form, $entry){
        ...
        foreach($donations as $donation){
            ...
            $purpose .= esc_html($name) . ", ";
            $price = GFCommon::to_number($price);
            $total += $price;
        }
        if(!empty($purpose))
            $purpose = urlencode(substr($purpose, 0, strlen($purpose)-2));
